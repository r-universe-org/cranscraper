  ...
  lapply(sample(which(!is.na(packages$Git))), function(i){
    ...
    curl::multi_add(make_handle(desc_url), done = function(res){
      if(res$status == 200 && test_package_match(res$content, package)){
        ...
        if(package == 'xgboost') alt_subdirs <- 'R-package'
        if(package == 'shinychat') alt_subdirs <- 'pkg-r'
        if(package == 'querychat') alt_subdirs <- 'pkg-r'
        if(package == 'commons') alt_subdirs <- 'pkg-r'
...
normalize_github_urls <- function(input){
  input <- gsub('gitlab.com/hrbrmstr/', 'github.com/hrbrmstr/', input, fixed = TRUE)
